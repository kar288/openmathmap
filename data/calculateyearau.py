	...
	if len(a) > 1:
		...
		for part in parts:
			...
			if p in authors:
				...
				authors[p]['minimum'] = min(authors[p]['minimum'], year)
				authors[p]['minimum'] = min(authors[p]['minimum'], year)
				for c in classes:
...
for author in authors:
	averageyearspan += authors[author]['minimum'] -authors[author]['maximum']
	averagecount += authors[author]['count']
	averageclasses += len(authors[author]['classes'])


averageyearspan /= len(authors)
averagecount /= len(authors)
averageclasses /= len(authors)

print averageyearspan, averagecount, averageclasses
# for author in authors:
# 	if len(author) > 0:
# 		# description = ""
